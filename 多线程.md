...
当 synchronized 同步普通方法时，一个对象只有一把锁，当一个线程获取了该对象的锁之后，其他线程无法获取该对象的锁，所以无法访问该对象的其他 synchronized 方法，但是其他线程还是可以访问该对象的其他非 synchronized 方法。
...
// schedule方法用于安排在给定延迟后执行一次任务，之后不再重复执行。
...
// scheduleAtFixedRate方法用于按固定频率重复执行任务，无论任务是否完成，都会在每个周期开始时执行。
...
// scheduleWithFixedDelay方法在每个任务完成后，等待一个固定的延迟时间后再执行下一个任务。
...
- CPU密集型任务
  对于CPU密集型任务，线程池的核心线程数设置应旨在最大化CPU的利用率，同时**避免过多的线程上下文切换带来的开销**。在这种情况下，核心线程数的设置可以有以下几种考虑：
...
- IO密集型任务
  ...
  - 2倍CPU核心数：这是一种常见的设置方式，可以确保**在IO操作等待期间，有足够多的线程在CPU上执行，从而提高系统的吞吐量**。
