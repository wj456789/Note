...
用于在内存达到最大限制时，决定哪些数据应该被移除。以下是Redis的几种淘汰策略：

- **noeviction**：当内存不足以容纳新写入数据时，新写入操作会报错。这种策略可以保护已有数据不被淘汰，但可能会导致一些写操作失败。
- **allkeys-lfu**：（Least Frequently Used）“最不经常使用”。LFU策略基于数据的访问频率，它记录每个键在特定时间窗口内被访问的次数，并选择访问次数最少的键进行淘汰。。
- **volatile-lfu**：只在设置了过期时间的键中使用LFU策略进行淘汰
- **allkeys-lru**：从所有键中使用LRU（Least Recently Used，最近最少使用）算法进行淘汰。LRU策略基于数据的访问时间，选择最长时间未被访问的数据进行淘汰。
- **volatile-lru**：从设置了过期时间的键中使用LRU算法进行淘汰。这种策略会优先淘汰设置了过期时间且最近最少使用的数据。
- **allkeys-random**：从所有键中随机淘汰数据。这种策略对于所有数据的淘汰概率是相等的，不考虑数据的使用频率或过期时间。
- **volatile-random**：从设置了过期时间的键中随机淘汰数据。这种策略会优先淘汰设置了过期时间的数据，但具体淘汰哪个数据是随机的。
- **volatile-ttl**：从设置了过期时间的键中淘汰具有更早过期时间的数据。这种策略会优先淘汰过期时间最早的数据，有助于避免数据过期后仍然占用内存。

要配置Redis的内存淘汰策略，可以在Redis的配置文件redis.conf中找到`maxmemory-policy`参数，并将其设置为所需的策略。例如，要将淘汰策略设置为`allkeys-lru`，可以将`maxmemory-policy`设置为`allkeys-lru`。在配置文件中，还可以设置`maxmemory`参数来限制Redis的最大内存使用量。当内存使用量达到这个限制时，Redis会根据配置的淘汰策略来移除数据。

需要注意的是，没有设置过期时间的key不会被淘汰，这样可以保证需要持久化的数据不会丢失。另外，虽然**LRU和LFU是常见的淘汰策略**，但它们并不适用于所有场景。在选择淘汰策略时，需要根据应用程序的特性和需求来进行权衡和选择。
...
**在查询代码中，当不存在插入一个空字符串到redis，设置过期时间，且插入代码时需要更新redis**
