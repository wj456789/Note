...
对于图书管理员，在系统提供借书服务基础上，有扩展预约借书的需求，而借书需要使用检查账号和记录借出记录的功能 

把书从系统借出-------include----->检查读者账户

把书从系统借出-------include----->记录借出记录

把书从系统借出<-------extend-----预约借书

### 类图

借阅者(Borrower) ------------Dependency ----------->书目标题（Title） 

借阅者(Borrower) ———— Association ————>预约记录(Reservation) 

借阅者(Borrower) ———— Association ————>借阅信息(BorrowedInfo) 

借阅信息(BorrowedInfo)◇1————  Aggregation————>0...n 借阅记录(Loan) 

借阅记录(Loan) ◆1————  Composition ————>1 书的相关信息(Item) i
