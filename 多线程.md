...
![clipboard](C:/Users/Administrator/Desktop/clipboard.png)
...
其中封装类里的变量都用了volatile保证内存可见，同时底层使用了CAS算法，保证操作原子性，代码效率比synchronized和Lock实现的线程同步都高，是一种无锁的**线程同步**。
...
- CAS也包含了读取、比较和写入这三个操作，但CAS通过硬件命令保证了原子性。
...
![img](img_%E5%A4%9A%E7%BA%BF%E7%A8%8B/executor%E7%B1%BB%E5%9B%BE.png)
...
public interface ExecutorService extends Executor {
    //调用该方法后，会平滑地关闭 ExecutorService，ExecutorService会停止接受任何新的任务且等待已经提交的任务执行完成
    void shutdown();
...
public interface Future<V> {
    //向线程发送一个中断信号，线程里面通过interrupt方法来接收信号手动return中断
    boolean cancel(boolean mayInterruptIfRunning);
    boolean isCancelled();
    boolean isDone();
    //调用get()可以获取结果，在这种情况下，get()将阻塞，直至结果准备就绪
...
6. rejectedExectutionHandler

   拒绝策略。rejectedExectutionHandler参数字段用于配置拒绝策略，常用拒绝策略如下
...
public interface ScheduledExecutorService extends ExecutorService {
    //接口定时方法
    //command：执行线程，initialDelay：初始化延时，period：每次执行的开始的最小间隔时间，unit：计时单位
    public ScheduledFuture<?> scheduleAtFixedRate(Runnable command,long initialDelay,long period,TimeUnit unit);
    
    //command：执行线程，initialDelay：初始化延时，period：前一次执行结束到下一次执行开始的间隔时间，unit：计时单位
