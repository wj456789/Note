...
？？？Innodb存储引擎select时默认不加共享，但是insert、update、delete操作会默认加排他锁。
...
- 间隙锁设计的目的是为了阻止多个事务将记录插入到同一范围内，而这会导致幻读问题的产生。两种方式显式关闭间隙锁：（除了外键约束和唯一性检查外，其余情况仅使用记录锁） A. 将事务隔离级别设置为Read Committed B. 将参数innodb_locks_unsafe_for_binlog设置为1
