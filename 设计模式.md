    ...
    try {
        if (parent.exists()) {
            is = new ObjectInputStream(new FileInputStream(
                ...
                .getRealPath("/") + "/autologinMap.dat"));
            Map<String, String[]> autologinMap = (ConcurrentHashMap<String, String[]>) is
                .readObject();
            AutoLoginMacMap.getInstance().setAutoLoginMacMap(autologinMap);
            if (BaseConfig.getInstance().getBaseConfig().getIsDebug()==1) {
...
private void autoAuthMapToDisk(ServletContextEvent servletContextEvent) {
...
private void autoAuthMapFromDisk(ServletContextEvent servletContextEvent) {
    ObjectInputStream is = null;
    File parent = new File(servletContextEvent.getServletContext()
                           .getRealPath("/") + "/autoAuthMap.dat");
    try {
        if (parent.exists()) {
            is = new ObjectInputStream(new FileInputStream(
                servletContextEvent.getServletContext()
            ...
            Map<String, String[]> autologinMap = (ConcurrentHashMap<String, String[]>) is
                .readObject();
            AutoLoginMap.getInstance().setAutoLoginMap(autologinMap);
            if (BaseConfig.getInstance().getBaseConfig().getIsDebug()==1) {
                logger.info("autoAuthMapFromDisk !!");
            }

        } else {
            if (BaseConfig.getInstance().getBaseConfig().getIsDebug()==1) {
                logger.info("autoAuthMap File Not Exists !!");
            }
        }
    } catch (Exception e) {
        logger.error("==============ERROR Start=============");
...
不改变现有对象结构的情况下，动态地给该对象增加额外功能的模式。使用组合来替代继承，装饰器类和原始类继承同样的父类
...
在对象之间有一个一对多的关系，当一个对象状态改变的时候，所有依赖的对象都会自动收到通知。多个对象注册，一个对象通知。
...
状态模式是状态机的一种实现方式即可。状态机又叫有限状态机，它有 3 个部分组成：状态、事件、动
作。其中，事件也称为转移条件。事件触发状态的转移及动作的执行。
